...
- [06-2.部署高可用 kube-apiserver 集群](#06-2部署高可用-kube-apiserver-集群)
    ...
    - [授予 kubernetes 证书访问 kubelet API 的权限](#授予-kubernetes-证书访问-kubelet-api-的权限)
    - [参考](#参考)
...
+ `--enable-admission-plugins`：启用 `ServiceAccount` 和 `NodeRestriction`；tips: latest release kubernetes 1.14.0 does not support Initializers (https://github.com/opsnull/follow-me-install-kubernetes-cluster/issues/440)
...
tcp        0      0 172.27.128.150:6443     0.0.0.0:*               LISTEN      51903/kube-apiserve
```
...
2. 创建 kube-apiserver systemd unit 模板文件中参数--enable-admission-plugins=Initializers报错Initializers is unknown: https://github.com/opsnull/follow-me-install-kubernetes-cluster/issues/440
